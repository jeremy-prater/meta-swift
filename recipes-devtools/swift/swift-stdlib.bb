...
set(SWIFT_STANDARD_LIBRARY_SWIFT_FLAGS -Xcc --gcc-install-dir=${STAGING_DIR_TARGET}/usr/lib/gcc/${TARGET_SYS}/${SWIFT_GCC_VERSION} -I${STAGING_DIR_TARGET}/usr/include/c++/${SWIFT_GCC_VERSION} -I${STAGING_DIR_TARGET}/usr/include/c++/${SWIFT_GCC_VERSION}/${TARGET_SYS} -no-verify-emitted-module-interface -Xcc -mthumb -Xcc -mfpu=neon -Xcc -mfloat-abi=hard -Xcc -mcpu=cortex-a15 -Xcc -D_TIME_BITS=64 -Xcc -D_FILE_OFFSET_BITS=64)
